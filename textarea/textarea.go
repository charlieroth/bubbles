...
func New() Model {
	...
	return Model{
		...
		CursorLineStyle:  lipgloss.NewStyle().Background(lipgloss.Color("235")),
		ShowLineNumbers:  false,

		id:               nextID(),
		value:            make([][]rune, maxHeight, maxWidth),
		focus:            false,
		blink:            true,
		col:              0,
		row:              0,
...
func (m *Model) Reset() bool {
	m.value = make([][]rune, maxHeight, maxWidth)
	m.col = 0
	m.row = 0
	m.viewport.GotoTop()
	return m.setCursor(0)
...
// lineDown moves the cursor down by one line.
...
func (m *Model) lineDown() {
	...
	for offset < charOffset {
		if m.col >= len(m.value[m.row]) || m.col >= nli.StartColumn+nli.Width {
			break
		}
...
// lineUp moves the cursor up by one line.
func (m *Model) lineUp() {
	li := m.LineInfo()
	charOffset := max(m.lastCharOffset, li.CharOffset)
	m.lastCharOffset = charOffset
...
func (m Model) View() string {
	...
	for l, line := range m.value {
		...
		for wl, wrappedLine := range wrappedLines {
			...
			padding := m.Width - rw.StringWidth(string(wrappedLine))
			if m.row == l && lineInfo.RowOffset == wl {
				if lineInfo.ColumnOffset >= lineInfo.Width {
					s.WriteString(style.Render(string(wrappedLine[:lineInfo.ColumnOffset]) + m.cursorView(" ")))
					padding--
				} else {
					s.WriteString(style.Render(string(wrappedLine[:lineInfo.ColumnOffset]) + m.cursorView(string(wrappedLine[lineInfo.ColumnOffset])) + style.Render(string(wrappedLine[lineInfo.ColumnOffset+1:]))))
				}
			} else {
...
func (m Model) cursorView(v string) string {
	if m.blink {
		return m.CursorLineStyle.Render(m.TextStyle.Render(v))
	}
	return m.CursorLineStyle.Render(m.CursorStyle.Inline(true).Reverse(true).Render(v))
}
